...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}p{2.8cm}XX@{}}
    ...
    将 refchecker 定位为生成期辅助工具，而不是最终 evaluator；最终错误率另建 \texttt{citation\_check} 流水线，由 claim--citation pair + 论文局部证据片段交给 judge 独立判定。 \\
    \addlinespace
    P1/P2 文献包不完全重合 &
    同一个 case 中 P1 和 P2 可能把 \texttt{[D]}、\texttt{[E]} 指向不同论文；如果只在 \texttt{papers/} 中保存一份 \texttt{[D].pdf}，后续 judge 会读错证据。 &
    将展示与评测材料整理到 \texttt{presentation/main\_memory/}；每个 pass 使用 pass-specific 文件名，例如 \texttt{P1\_[D]\_2405.17739.pdf} 与 \texttt{P2\_[D]\_2410.12944.md}，脚本按 \texttt{case + pass + tag} 精确匹配。 \\
    \addlinespace
    LLM judge 成本和速度 &
...
因此，最终可复现材料不只包括报告输出，还包括 profile audit、run manifest、stderr、memory snapshot、pass-specific papers、claim--citation pair JSONL 和 judge batch prompt。这些文件共同构成了本实验的复现边界。

% ===================================================================
\section{现场展示工具包}
% ===================================================================

为现场展示准备的 demo 工具包位于 \texttt{demo/} 目录：

\begin{verbatim}
demo/
  README.md     -- 使用说明
  setup.sh      -- 一键配置环境
  run.sh        -- 运行入口（接受 --pdf 和 --topic）
  outputs/      -- 输出目录
\end{verbatim}

使用方式：
\begin{verbatim}
bash demo/setup.sh                          # 首次配置
bash demo/run.sh --pdf paper.pdf --topic "..."  # 运行
\end{verbatim}

Agent 自动完成：读 PDF → 检索补充论文 → 生成 CPS 格式报告 → refchecker 核查 → 修复后报告。
