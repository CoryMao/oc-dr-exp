...
在 `citation-standard/` 下维护 3 个运行时文件，并在需要时安装到 OpenClaw skills 目录：
...
**路径：** `citation-standard/references/cps-spec.md`
...
**路径：** `citation-standard/scripts/validate.py`
...
推荐通过 OpenClaw CLI 安装本地 skill：

```bash
openclaw skills install /path/to/citation-standard --as citation-standard --global --force
```

或者在 `~/.openclaw/openclaw.json` 的 `skills.entries` 中添加条目，Agent 即可按需加载引用格式规范：
